...
const RightSIdebar = () => {
  ...
  return selectedUser && (
    <div className={`bg-[#8182B5]/10 text-white w-full relative overflow-y-scroll
    ${selectedUser ?'max-md:hidden' :'' }
    `}>
      <div className='pt -16 flex flex-col items-center gap-2 text-xs font-light mx-auto'>
        <img src={selectedUser?.profilePic || assets.avatar_icon} alt="" className='w-25 aspect-[1/1] rounded-full m-7' />
      ...
      text-sm font-light py-2 px-20 rounded-full cursor-pointer' onClick={()=>logout()}>Logout</button>
    </div>
  )
}
