  ...
  return (
    <div className='min-h-screen bg-cover bg-no-repeat flex items-center justify-center'>
      ...
      flex items-center justify-between max-sm:flex-col-reverse rounded-lg'>
        ...
        action="" className='flex flex-col gap-5 p-10 flex-1'>
          <h3 className='text-lg'>Profile details</h3>
          <label htmlFor="avatar" className='flex items-center gap-3 cursor-pointer'>
            <input 
            onChange={(e)=>setSelectedImage(e.target.files[0])}
            type="file" id='avatar' accept='.png, .jpg, .jpeg' hidden/>
          <img src={selectedImage ? URL.createObjectURL(selectedImage) : authUser?.profilePic ? authUser.profilePic : assets.avatar_icon} alt=""
          ...
          value={name}
          type="text" required placeholder='Enter your name :' className='p-2 border border-gray-500 rounded-md 
          focus:outline-none focus:ring-2 focus:ring-violet-500'/>
          <textarea 
          onChange={(e)=>setBio(e.target.value)}
          value={bio}
          name="" id="" placeholder='Write profile bio' required
